...
from mmpose.apis import (get_track_id, inference_top_down_pose_model, init_pose_model, process_mmdet_results, vis_pose_result)
...
def main():
    ...
    for video in os.listdir(args.video_path):
        if os.path.exists(os.path.join(args.out_pose_root, f'{video.replace(".", "_", 1).split(".")[0]}_pose_sequence.npy')):
            logger.info(f'Skipping {video}, already loaded')
            continue
        logger.info(f'Loading {video}')
        ...
        while (cap.isOpened()):
            ...
            # logger.info(f'Frame: {frame_count-1}, Poses: {len(pose_results)}, Total: {len(poses)}')
            # show the results
