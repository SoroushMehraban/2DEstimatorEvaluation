...
if action_filter is not None:
    print('Selected actions:', action_filter, flush=True)

cameras_valid, poses_valid, poses_valid_2d = fetch(subjects_test, action_filter)

receptive_field = args.number_of_frames
print('INFO: Receptive field: {} frames'.format(receptive_field), flush=True)
...
print('INFO: Trainable parameter count:', model_params, flush=True)
...
if args.resume or args.evaluate:
    ...
    print('Loading checkpoint', chk_filename, flush=True)
...
print('INFO: Testing on {} frames'.format(test_generator.num_frames()), flush=True)
    ...
    for epoch in tqdm(range(start_epoch, args.epochs)):
        ...
        if args.no_eval:
            print('[%d] time %.2f lr %f 3d_train %f' % (
                ...
                elapsed,
                lr,
                losses_3d_train[-1] * 1000), flush=True)
        else:
...
def evaluate(test_generator, action=None, return_predictions=False, use_trajectory_model=False):
    ...
    print('Protocol #1 Error (MPJPE):', e1, 'mm', flush=True)
    print('Protocol #2 Error (P-MPJPE):', e2, 'mm', flush=True)
    print('Protocol #3 Error (N-MPJPE):', e3, 'mm', flush=True)
    if args.dataset.startswith('mpi'):
        e4 = (epoch_loss_pck / N)
        e5 = (epoch_loss_auc / N)
        print('Protocol #4 PCK:', e4, flush=True)
        print('Protocol #5 AUC:', e5, flush=True)
    print('Velocity Error (MPJVE):', ev, 'mm', flush=True)
    print('----------', flush=True)
...
if args.render:
    ...
    if ground_truth is None:
        print('INFO: this action is unlabeled. Ground truth will not be rendered.', flush=True)
    ...
    if args.viz_export is not None:
        print('Exporting joint positions to', args.viz_export, flush=True)
    ...
    def run_evaluation(actions, action_filter=None):
        ...
        print('Protocol #1   (MPJPE) action-wise average:', round(np.mean(errors_p1), 1), 'mm', flush=True)
        print('Protocol #2 (P-MPJPE) action-wise average:', round(np.mean(errors_p2), 1), 'mm', flush=True)
        print('Protocol #3 (N-MPJPE) action-wise average:', round(np.mean(errors_p3), 1), 'mm', flush=True)
        if args.dataset.startswith('mpi'):
            print('PCK:', round(np.mean(errors_p4), 1), flush=True)
            print('AUC:', round(np.mean(errors_p5), 1), flush=True)
        print('Velocity      (MPJVE) action-wise average:', round(np.mean(errors_vel), 2), 'mm', flush=True)
    ...
    if not args.by_subject:
        ...
        for subject in all_actions_by_subject.keys():
            print('Evaluating on subject', subject, flush=True)
